...
## 사전지식
### 파이어베이스
- 모바일 앱 개발에 최적화된 기능을 제공
그림
### 파이어스토어
- NoSQL로 파이어 베이스에서 제공
- 컬렉션(Talbe), 문서(Row) 으로 구성.
- get or snapshot, add or set, delete, update
...
- minSdkVersion 21
### 파이어베이스 CLI 설치 및 로그인
- firebase.google.com/docs/cli#install-cli-windows 설치 및 로그인
- 터미널에서 FlutterFire CLI 커맨드로 설치
  dart pub global activate flutterfire_cli
  Failed to decode data using encoding 'utf-8' 오류시 dart pub cache repair 실행 후 재실행
- console.firebase.google.com 프로젝트 생성 
- flutterfire configure -p 프로젝트id 설정
- console.firebase.google.com 파이어데이터베이스 설정 및 규칙  allow read, write: if true;
